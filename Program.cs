...
namespace mineproxy
{
	...
	public class MP
	{
		public string tarrgett = "";
		public static int buffviewscounter = 0;
		...
		public static ConcurrentBag<string> live = new();
		public List<string> FetchedProxies = new(70000);
		public List<string> VipProxies = new(10000);
		public async Task Init(string tg)
		{
			this.tarrgett = tg;
			int c;
			string content;
			...
			foreach (var ProxySource in ProxySources)
			{
				...
				if ((int)res.StatusCode == 200)
				{
					...
					Console.WriteLine($"\x1b[33mFetched {lines.Length} proxies!\x1b[0m");
					c = 0;
					...
					Console.WriteLine($"\x1b[32mAdded {c} valid proxies!\x1b[0m");
				} else 
				{
		...
		public async Task FirstRun(int workerCount)
		{
			if (FetchedProxies.Count == 0) throw new InvalidOperationException("No proxy available");
		...
		public async Task Run(int workerCount)
		{
			...
			await Task.WhenAll(workers);
			List<string> livesibu = live.ToList();
			foreach (var p in livesibu) if (!VipProxies.Contains(p)) VipProxies.Add(p);
		}
		public async Task Run2(int workerCount)
		{
			queue = new ConcurrentQueue<string>();
			live = new ConcurrentBag<string>();
		...
		public async Task Worker(int id)
		{
			...
			while (queue.TryDequeue(out string? proxy))
			{
				...
				try
				{
					...
					var res = await proxiedClient.PostAsync(this.tarrgett, content);
					//var res = await proxiedClient.PostAsync("https://api.scratch.mit.edu/users/thanh_cundz/projects/937306446/views", content);
					int statcode = (int)res.StatusCode;
					Console.Write($"\r\x1b[32m[WORKER {idvip}] LIVE {statcode} {proxy}\x1b[0m           \nBV Counter: {buffviewscounter}");
					if (statcode == 200) {
						Interlocked.Increment(ref buffviewscounter);
						live.Add(proxy);
					}
				}
				catch
				{
					Console.Write($"\r\x1b[31m[WORKER {idvip}] DEAD --- {proxy}\x1b[0m           \nBV Counter: {buffviewscounter}");
				}
			}
		}
		public async Task Geonode()
		{
			...
			if ((int)res.StatusCode == 200)
			{
				...
				Console.WriteLine($"\x1b[33mFetched {data.GetArrayLength()} proxies!\x1b[0m");
